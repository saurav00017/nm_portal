...
import yaml
import jwt
from .task import async_task_infosys_file_upload


@api_view(['POST'])
@authentication_classes([JWTTokenUserAuthentication])
@permission_classes([IsAuthenticated])
def upload_infosys_file(request):
    ...
    if csv_file is None:
        content = {
            "message": "Please provide csv_file"
        }
    ...
    if account_role in [AccountRole.NM_ADMIN, AccountRole.NM_ADMIN_STAFF]:
        try:
            ...
            async_task_infosys_file_upload.delay(new_course_file.id)
            content = {
...
@authentication_classes([JWTTokenUserAuthentication])
@permission_classes([IsAuthenticated])
def infosys_file_upload_list(request):
    token = request.headers['Authorization']
    decoded_data = jwt.decode(token[7:], settings.JWT_SECRET_KEY, algorithms=['HS256'])
    account_role = decoded_data.get('account_role', None)
    ...
    if account_role in [AccountRole.NM_ADMIN, AccountRole.NM_ADMIN_STAFF]:
        try:
            course_file_upload_list = CourseBulkUpload.objects.filter(**query, course_type=2).order_by('-created')
