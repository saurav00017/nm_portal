...
import yaml
import jwt
from .celery_task import async_task_coursera_file_upload


@api_view(['POST'])
@authentication_classes([JWTTokenUserAuthentication])
@permission_classes([IsAuthenticated])
def upload_coursera_file(request):
    ...
    if skill_offering_id is None or csv_file is None:
        content = {
            "message": "Please provide skill_offering_id/ csv_file"
            }
    ...
    if account_role in [AccountRole.NM_ADMIN, AccountRole.NM_ADMIN_STAFF]:
        try:
            ...
            new_coursera_file = CourseBulkUpload.objects.create(
            ...
            async_task_coursera_file_upload.delay(new_coursera_file.id)
            content = {
...
@authentication_classes([JWTTokenUserAuthentication])
@permission_classes([IsAuthenticated])
def coursera_file_upload_list(request):
    token = request.headers['Authorization']
    decoded_data = jwt.decode(token[7:], settings.JWT_SECRET_KEY, algorithms=['HS256'])
    account_role = decoded_data.get('account_role', None)
    ...
    if account_role in [AccountRole.NM_ADMIN, AccountRole.NM_ADMIN_STAFF]:
        try:
            coursera_file_upload_list = CourseBulkUpload.objects.filter(**query, course_type=0).order_by('-created')
            total_count = coursera_file_upload_list.count()
            if view_all == '1':
                limit = total_count

            final_list = []
            for coursera in coursera_file_upload_list:
                temp = {
                    'coursera_file_upload_id': coursera.id,
                    'status': coursera.status,
                    'error_message': coursera.error_message,
                    'created': coursera.created,
                    'updated': coursera.updated,
                    'skill_offering_id': coursera.skill_offering_id,
                    'course_name': coursera.skill_offering.course_name if coursera.skill_offering_id else None,
                    'knowledge_partner_id': coursera.skill_offering.knowledge_partner_id if coursera.skill_offering_id else None,
                    'knowledge_partner': (coursera.skill_offering.knowledge_partner.name if coursera.skill_offering.knowledge_partner_id else None) if coursera.skill_offering_id else None,
