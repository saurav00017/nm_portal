...
fcount=0
ucount=0
with open(os.path.join(settings.BASE_DIR, "scripts/mandatory/15oct.csv"), 'r') as file:
    ...
    for record in csv_data:
        ...
        if college_info is not None and branch_info is not None and course_info is not None:
            try:
                old_course = MandatoryCourse.objects.get(
                    college_id = college_info.id,
                    skill_offering_id = course_info.id,
                    branch_id = branch_info.id,
                    sem = sem )
                old_course.course_type = course_type
                old_course.is_unlimited = is_unlimited
                old_course.count = allocations_count
                old_course.save()
                ucount = ucount + 1
...
print(tcount,scount,fcount,ucount,sep=",")
2886,2340,0,546