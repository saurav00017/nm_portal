...
file = open(os.path.join(settings.BASE_DIR, 'scripts/mandatory/data/by_college_code/infosys/oct20_data.csv'))
csv_data = csv.reader(file)

export_file = open(os.path.join(settings.BASE_DIR, 'scripts/new/lms/cleanup-raw4.csv'), 'w')
            ...
            for sem in mandatory_sems:
                ...
                for student in b_s_students:
                    try:
                        ...
                        try:
                            subscription = StudentCourse.objects.get(is_mandatory=1, student_id=student.id,course_id=check_sf_e.skill_offering.lms_course_id,status=1)
                            if subscription:
                                subscription.temp_z = 1
                                subscription.save()
                            ...
                            sub_count = sub_count + 1
                            subscriptions = StudentCourse.objects.filter(is_mandatory=1, student_id=student.id,course_id=check_sf_e.skill_offering.lms_course_id,status=1)
                            dup_sub = dup_sub + subscriptions.count()
                            subscriptions.update(temp_z=1)
                        print(sub_count)
                        print(dup_sub)
