...
counter = {}
student_unique_ids = list(enrollment_progress_list.values_list('skill_offering_enrollment__student__invitation_id', flat=True))

with open('/opt/nm/NM_portal_backend/nm_portal/scripts/new/lms/ingage/student_ids1.csv', 'w') as f:
    f.write(str(','.join(student_unique_ids)))

unique_count = unique_count.count()
