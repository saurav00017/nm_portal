...
file = open(os.path.join(settings.BASE_DIR, 'scripts/courses/f1_paid_courses.csv'), 'r')
...
for record in csv_data:
    ...
    new_Skil_offering = SKillOffering.objects.create(
        ...
        link=record[14],
        is_mandatory=0,
        offering_type=0 if record[15].lower() == 'offline' else 1,
        offering_kind=1,
    ...
    if len(branches) == 0:
        ...
        for branch in all_branchs:
            if not branch == '' or branch is None:
