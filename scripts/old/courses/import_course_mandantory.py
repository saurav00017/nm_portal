...
file = open(os.path.join(settings.BASE_DIR, 'scripts/courses/f1_mandatory_courses.csv'), 'r')
...
for record in csv_data:
    ...
    new_Skil_offering = SKillOffering.objects.create(
        ...
        mode_of_delivery=record[9],
        duration=record[10],
        outcomes=record[11],
        course_content=None,
        description=None,
        certification=record[12],
        cost=record[14],
        link=record[15],
        sem=record[8],
        is_mandatory=1,
        offering_type=0 if record[9].lower() == 'offline' else 1,
        offering_kind=1,
        job_category=record[13],
    )
    new_Skil_offering.save()

    specialization = record[4]
    specialization_info = Specialisation.objects.get(id=specialization)
